...
import type { RegisterUserFormData } from "../lib/marketing-schema";

// Mock API functions
const registerUser = async (data: RegisterUserFormData): Promise<{ success: boolean; id: string }> => {
  
    console.log("Marketing/ Affiliate Data:", data)
    // Simulate API delay
...
export const useRegisterUser = () => {
  ...
  return useMutation({
    ...
    onSuccess: (data) => {
      addReferral(data)
      incrementSignups()
      addRewards(100) // Add 100 B.C for each successful referral
