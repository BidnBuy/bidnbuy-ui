...
const WithdrawalSuccessView = ({ amount, transactionId, newBalance }: WithdrawalSuccessProps) => {
  ...
  const { lastTransaction, currentBalance } = useWithdrawalStore()

  // Use props or fallback to store data
  const displayAmount = amount || lastTransaction?.amount?.toString() || "0"
  const displayTxnId = transactionId || lastTransaction?.transactionId || "TXN000000000"
  const displayBalance = newBalance || lastTransaction?.newBalance?.toString() || "0"
  ...
  return (
    <div className="w-full max-w-md mx-auto md:max-w-lg text-center">
      ...
      <p className="text-gray-300 text-lg mb-8">
        You have successfully withdrawn ₦{currentBalance.toLocaleString()} to your bank account.
      </p>
