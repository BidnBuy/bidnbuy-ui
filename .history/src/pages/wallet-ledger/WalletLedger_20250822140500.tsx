...
const WalletLedger = () => {
  ...
  const { currentBidCreditBalance, setCurrentBalance } = useBidCreditStore(
    (state) => ({ 
      currentBidCreditBalance: state.currentBalance,
      setCurrentBalance: state.setCurrentBalance
    })
  );
    ...
    <div className="min-h-screen" style={{ backgroundColor: "#01151C" }}>
      <main className="px-4 py-6 md:px-8 md:py-12">
        <div className="max-w-md mx-auto md:max-w-3xl lg:max-w-4xl">
          ...
          />
          <BalanceSummary
            walletBalance={`N${currentBidCreditBalance.toLocaleString()}`}
