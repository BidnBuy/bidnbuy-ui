...
interface PromoteAuctionModalProps {
  open: boolean
...
export function PromoteAuctionModal({ open, onOpenChange, auctionId, auctionTitle }: PromoteAuctionModalProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-slate-800 border-slate-700 text-white max-w-sm mx-auto">
        <DialogHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          <DialogTitle className="text-lg font-medium">Promote Auction</DialogTitle>
          <Button
            variant="ghost"
            size="sm"
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              <Input
                value={referralLink}
                readOnly
                className="bg-slate-700 border-slate-600 text-slate-300 text-sm flex-1"
              />
              <Button
                onClick={handleCopyLink}
                size="sm"
                className={`px-3 py-2 text-xs ${
                  linkCopied ? "bg-green-600 hover:bg-green-500" : "bg-teal-600 hover:bg-teal-500"
                }`}
              >
                {linkCopied ? (
                  "Link copied"
