...
const NewCompleteProfilePage() {
  ...
  const onSubmit = async (data: ProfileFormData) => {
    ...
    try {
      ...
      const response = await axios.post("/api/v1/user/complete-profile", payload, {
        headers: {
          ...
          <div className="bg-teal-800/30 backdrop-blur-sm rounded-lg p-6">
            ...
            <div className="mt-8">
              <button
                ...
                className="w-full font-medium py-3 px-6 rounded-md transition-colors text-white hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isSubmitting ? "Updating..." : isEditing ? "Complete Profile & Create Wallet" : "Update"}
              </button>
            </div>
          </div>
        </form>
