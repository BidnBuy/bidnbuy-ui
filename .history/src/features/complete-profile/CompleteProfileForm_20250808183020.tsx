...
export function CompleteProfileForm({ initialData, onSuccessRedirect }: CompleteProfileFormProps) {
  ...
  const handleDateChange = (date: Date | undefined) => {
    if (date && !isNaN(date.getTime())) {+   setValue("dateOfBirth", format(date, "dd-MM-yyyy"), { shouldValidate: true });
+ } else {
          ...
          <div className="space-y-2">
            ...
            <Popover>
              ...
              <PopoverContent className="w-auto p-0">
                <Calendar
                  mode="single"
                  selected={watchedDateOfBirth ? new Date(watchedDateOfBirth.split('-').reverse().join('-')) : undefined}
