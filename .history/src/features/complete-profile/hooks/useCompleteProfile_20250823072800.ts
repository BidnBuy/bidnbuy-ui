...
export const useCompleteProfile = () => {
  ...
  const mutation = useMutation({
    mutationFn: async (payload: CompleteProfilePayload) => authService.profile(payload),
    onSuccess: async (result) => {
      // After profile completion, check wallet status
      try {
        const walletRes = await import("@/services/wallet").then(response => response.getWalletStatus());
