...
type AuctionDataStatus = typeof AuctionDataStatus[keyof typeof AuctionDataStatus]
...
//   joinBid: (auctionId: string, bidAmount: number) => void
// }
...
export const useAuctionStore = create<AuctionState>()(
  ...
  auctions: mockAuctions,
        ...
        getFilteredAuctions: () => {
          ...
          // Adjust filter logic as needed for your AuctionData
          return auctions.filter((auction) => {
            switch (filter) {
              case "Ending Soon":
                // Example: auctions ending in <= 6 hours
                const msLeft = auction.endTime.getTime() - Date.now()
                return msLeft > 0 && msLeft <= 6 * 60 * 60 * 1000
              case "New Listings":
                // Example: started within last 24h
                return Date.now() - auction.startTime.getTime() <= 24 * 60 * 60 * 1000
              case "Popular":
