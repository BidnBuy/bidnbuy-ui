...
import type { BaseManagementItem, BidData, ManagementPageConfig } from "@/types/admin-management"
...
const pageConfig: ManagementPageConfig = {
  ...
  mobileCard: (item) => {
    const bid = item as BidData;
    return {
    title: bid.bidder,
    subtitle: `Bid Amount: ₦${bid.bidAmount.toLocaleString()}`,
    additionalInfo: [
    `Product: ${bid.product}`,
    `Time: ${bid.bidTime}`,
    `Status: ${bid.status}`,
...
const AdminBidManagement = () => {
  ...
  const handleViewBid = (item: BaseManagementItem) => {
    const bid = item as BidData
